...
TH2 *treeToHist2D(TTree *t, TString x, TString y, TString name, TCut cut, double xmin, double xmax, double ymin, double ymax, int xbins, int ybins, double minv)
{
    ...
    for (int ix = 1; ix <= xbins; ++ix)
    {
        for (int iy = 1; iy <= ybins; ++iy)
        {
            ...
            if (z == 0 && (prof->GetBinContent(ix + 1, iy) != 0 || prof->GetBinContent(ix, iy + 1) != 0 || prof->GetBinContent(ix - 1, iy) != 0 || prof->GetBinContent(ix, iy - 1) != 0))
            {
                double row = (prof->GetBinContent(ix + 1, iy) + prof->GetBinContent(ix - 1, iy)) / 2;
                double column = (prof->GetBinContent(ix, iy + 1) + prof->GetBinContent(ix, iy - 1)) / 2;
                // z=(row+column)/2;
                z = (row > column) ? row : column;
                if (iy == 1 || iy == ybins)
                {
                    if (prof->GetBinContent(ix + 1, iy) != 0 && prof->GetBinContent(ix - 1, iy) != 0)
                        z = row;
                    else if (prof->GetBinContent(ix + 1, iy) != 0)
                        z = prof->GetBinContent(ix + 1, iy);
                    else if (prof->GetBinContent(ix - 1, iy) != 0)
                        z = prof->GetBinContent(ix - 1, iy);
                    else if (iy == 1)
                        z = prof->GetBinContent(ix, iy + 1);
                    else
                        z = prof->GetBinContent(ix, iy - 1);
                }
                else if (iy == 0 || iy == ybins)
                {
                    z = column;
                }
            }
            else if (z == 0 && prof->GetBinContent(ix, iy + 1) != 0)
                z = (prof->GetBinContent(ix, iy - 1) + prof->GetBinContent(ix, iy + 1)) / 2;
...
void contour2D(TString dir, TString datafile, TString xvar, int xbins, float xmin, float xmax, TString yvar, int ybins, float ymin, float ymax, float smx = 1.0, float smy = 1.0, TString name = "contour2D", TString xtitle = "#mu_{ggH,bbH,ttH}", TString ytitle = "#mu_{VBF,VH}", TFile *fOut = 0)
{
    ...
    //
    // CMS_lumi( c, 4, 0, true);
    TLegend *leg = new TLegend(0.66, 0.70, 0.86, 0.88);
