...
echo "output: $output"
wrong="f"
cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/2016/condor_out_corr/$1
file=$(ls ${1}.txt)
dir_f=$(cat $file)
#dir="root://cms-xrd-global.cern.ch/"$dir
dir="root://xrootd-cms.infn.it/"$dir_f
eos="/eos/user/y/yuekai/ttbar/select/2016/corr"
inputFile=${file%.txt*}
...
mv $output/out*.txt /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/2016/condor_out_corr/$1
if [[ $wrong == "f" ]]
then
    echo "input file: $dir"
    input=$(ls $output|grep root)
    cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/select
    root -l -q -b ./process_corr.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",2016)"
