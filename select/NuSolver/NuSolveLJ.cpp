...
// init.
NuSolveLJ::NuSolveLJ() : Unit_(make_unit()), Deriv_(make_deriv()), nu_(0, 0, 0, -1)
{
...
void NuSolveLJ::MakeSolution(const TLorentzVector &bj, const TLorentzVector &lep, double met_pt, double met_phi, double metunc_x, double metunc_y, double metxy_rho)
{
    Reset();
    BuildH(bj, lep);
    ...
    std::cout << endl
              ...
              << H_ << endl;
#endif
    SetMET(met_pt, met_phi, metunc_x, metunc_y, metxy_rho);
    Solve();
#ifdef DEBUG_NuSolveLJ
    std::cout << endl
              << "MET: " << endl
...
void NuSolveLJ::BuildH(const TLorentzVector &bjet, const TLorentzVector &lep)
{
    Ml_ = lep.M();
    Mb_ = bjet.M();
    // Mn_ = 0.;
    Eigen::Vector3d l(lep.Px(), lep.Py(), lep.Pz());
    Eigen::Vector3d b(bjet.Px(), bjet.Py(), bjet.Pz());
...
std::vector<Soln2d> NuSolveLJ::IntersectLineEllipse(const Eigen::Vector3d &L, const Eigen::Matrix3d &ellipse)
{
#ifdef DEBUG_NuSolveLJ
    std::cout << endl
              << "NuSolveLJ::IntersectLineEllipse(L, M) " << endl
              << L << endl
              << ellipse << endl;
    std::cout << "-----" << endl;
#endif
    ...
    for (int i = 0; i < 3; ++i)
    {
        if (abs(eivals(i).imag()) > 1e-6 || abs(eivals(i).real()) < 1e-9)
        {
            continue;
        }
...
std::vector<Soln2d> NuSolveLJ::IntersectEllipses(const Eigen::Matrix3d &ellipse1, const Eigen::Matrix3d &ellipse2)
{
    ...
    for (int evindex = 0; evindex < 3; evindex++)
    {
        ...
        if (abs(eivals(evindex).imag()) > 10e-10)
        {
            continue;
        }
...
void NuSolveLJ::Solve()
{
    ...
    auto best = min_element(
        solns2d.begin(), solns2d.end(), [X, this](const Soln2d &a, const Soln2d &b) -> bool
        { return Chi2(X, a.vec) < Chi2(X, b.vec); });

    nu_chisq_ = Chi2(X, best->vec);
    soln = H_ * best->vec;
    nu_.SetPxPyPzE(soln(0), soln(1), soln(2), soln.norm());
    solved_ = true;
}

// set the MET

void NuSolveLJ::SetMET(double met_pt, double met_phi, double metxerr, double metyerr, double metxyrho)
{
    MET_(0, 0) = met_pt * cos(met_phi);
    MET_(1, 0) = met_pt * sin(met_phi);

    V0_(0, 2) = met_pt * cos(met_phi);
    V0_(1, 2) = met_pt * sin(met_phi);
