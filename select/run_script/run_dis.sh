...
echo "output: $output"
wrong="f"
cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/20${year}/condor_out_dis/$1
file=$(ls ${1}.txt)
dir_f=$(cat $file)
#dir="root://cms-xrd-global.cern.ch/"$dir
dir="root://xrootd-cms.infn.it/"$dir_f
eos="/eos/user/y/yuekai/ttbar/select/20${year}/dis"
inputFile=${file%.txt*}
...
mv $output/out*.txt /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/20${year}/condor_out_dis/$1
if [[ $wrong == "f" ]]
then
    echo "input file: $dir"
    ...
    root -l -q -b ./process_dis.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",20${year})"
