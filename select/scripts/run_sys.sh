...
echo "output: $output"
wrong="f"
cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/20${2}/condor_out_sys/$1
file=$(ls ${1}.txt)
dir_f=$(cat $file)
#dir="root://cms-xrd-global.cern.ch/"$dir
dir="root://xrootd-cms.infn.it/"$dir_f
eos="/eos/user/y/yuekai/ttbar/select/20${2}/MC_sys"
inputFile=${file%.txt*}
...
mv $output/out*.txt /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/20${2}/condor_out_sys/$1
if [[ $wrong == "f" ]]
then
    echo "input file: $dir"
    ...
    do
        root -l -q -b ./SF_add.cpp"(\"$outputFile\",1,20${2},"A")"
