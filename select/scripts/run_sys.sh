...
echo "output: $output"
wrong="f"
cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/20${year}/condor_out_sys/$1
file=$(ls ${1}.txt)
dir_f=$(cat $file)
#dir="root://cms-xrd-global.cern.ch/"$dir
dir="root://xrootd-cms.infn.it/"$dir_f
eos="/eos/user/y/yuekai/ttbar/select/20${year}/MC_sys"
inputFile=${file%.txt*}
...
mv $output/out*.txt /afs/cern.ch/user/y/yuekai/EFT-ttbar/select/20${year}/condor_out_sys/$1
if [[ $wrong == "f" ]]
then
    echo "input file: $dir"
    ...
    root -l -q -b ./process.cpp"(\"$output\",\"$inputFile\",\"$output/$input\",20${year},2)"
    cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/scale_factor/code
    for outputFile in $(ls $output/new*.root)
    do
        root -l -q -b ./SF_add.cpp"(\"$outputFile\",1,20${year})"
    ...
    then
        ...
        for outputFile in $(ls $output/new*.root)
        do
            root -l -q -b ./nnlo_add.cpp"(\"$outputFile\",1, 20${year})"
        done
        cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/EW_weight
        for outputFile in $(ls $output/new*.root)
        do
            root -l -q -b ./add_weight_branch.cpp"(\"$outputFile\",1, 20${year})"
        done
    fi
    cd /afs/cern.ch/user/y/yuekai/EFT-ttbar/pileup
    for outputFile in $(ls $output/new*.root)
    do
        root -l -q -b add_pu.cpp"(\"$outputFile\",20${year},1)"
    done
